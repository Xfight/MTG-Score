...
- (void)retryToRetrieveAd {
	// adMobAd e' nil in quanto questo metodo viene invocato quando e' nil !
	CGSize sizeToRequest = self.view.frame.size;
	adMobAd = [AdMobView requestAdOfSize:sizeToRequest withDelegate:self]; // start a new ad request
	[adMobAd retain]; // this will be released when it loads (or fails to load)
...
- (NSString *)publisherIdForAd:(AdMobView *)adView {
  return @"a14d70a3e5408ca"; // this should be prefilled; if not, get it from www.admob.com
}
...
- (void)didFailToReceiveAd:(AdMobView *)adView {
  //NSLog(@"AdMob: Did fail to receive ad in AdViewController");
    ...
    /*
    id tfu = currentViewController;
    NSDate* date = [NSDate date];
    NSDateFormatter* formatter = [[[NSDateFormatter alloc] init] autorelease];
    [formatter setDateFormat:@"HH:MM:SS"];
    NSString* str = [formatter stringFromDate:date];
    [[tfu tmpButtonForAd] setTitle:[NSString stringWithFormat:@"[%@] adMob errore !!", str]
                          forState:UIControlStateNormal];*/
    
  [adMobAd release];
  adMobAd = nil;
	if ( timer == nil )
		timer = [[NSTimer scheduledTimerWithTimeInterval:30 target:self selector:@selector(retryToRetrieveAd) userInfo:nil repeats:YES] retain];
